...
export function FilterModal({ open, onClose, title }: IModal) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<CreateSearchFilterData>({
      ...
      <form onSubmit={handleSubmit(createFilter)}>
        <div className="flex items-center gap-5 flex-1 mt-4">
          <div className="w-full min-h-[90px]">
            ...
            />
            {errors.brand && (
              <span className="text-carmesim-500">{errors.brand.message}</span>
            )}
          </div>
          ...
          <div className="w-full min-h-[90px]">
            ...
            />
            {errors.year && (
              <span className="text-carmesim-500">{errors.year.message}</span>
            )}
          </div>
        </div>

        <div className="w-full flex items-center gap-4 justify-end mt-6">
          <button className="bg-white text-carmesim-500 border border-carmesim-500 text-base rounded px-4 py-1">
            Cancelar
          </button>
          <button
            className="bg-carmesim-500 text-white font-bold text-base rounded px-4 py-1"
            type="submit"
          >
            Filtrar
          </button>
